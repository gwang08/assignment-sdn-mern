...
exports.getStudentHealthProfile = async (req, res) => {
  try {
    ...
    const formattedProfile = {
      ...
      chronic_conditions:
        profile.chronicDiseases?.map((disease) => disease.name) || [],
      chronicDiseases: profile.chronicDiseases || [],

      // Medical history from treatment history
      medical_history:
        profile.treatmentHistory?.map(
          (treatment) =>
            `${treatment.condition} - ${
      ...
      // Vision status
      vision_status:
        profile.vision && (profile.vision.leftEye || profile.vision.rightEye)
          ? `Mắt trái: ${profile.vision.leftEye || "N/A"}/10, Mắt phải: ${
              profile.vision.rightEye || "N/A"
            }/10`
          : "Chưa kiểm tra",
      ...
      // Hearing status
      hearing_status:
        profile.hearing && (profile.hearing.leftEar || profile.hearing.rightEar)
          ? `Tai trái: ${profile.hearing.leftEar || "N/A"}, Tai phải: ${
              profile.hearing.rightEar || "N/A"
            }`
          : "Chưa kiểm tra",
...
exports.getStudentMedicalEvents = async (req, res) => {
  try {
    ...
    const formatted = events.map((event) => ({
      ...
      medications_given: (event.medications_administered || []).map(
        (m) => m.name
      ),
      status: event.status.toLowerCase(),
      created_by: event.created_by?._id || "",
      follow_up_required: event.follow_up_required || false,
      follow_up_date: event.follow_up_notes
